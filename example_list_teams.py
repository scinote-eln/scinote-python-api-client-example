...
# Finally the API part, access the teams
# Mind that url is defined to catch teams from teams with id 1
# You can change this if you wish
api_url = settings['server_url'] + "/api/v1/teams/"
...
try:
    ...
    print("HTTP request failed when fetching teams")
    quit()

try:
    json_api_response = json.loads(api_response.read().decode())
except:
    print("Parsing response into json failed when fetching teams")
    quit()

print("Listing teams")
print("-------------------")
for team in json_api_response['data']:
    print(team['id'] + ": " + team['attributes']['name'])
print("-------------------")